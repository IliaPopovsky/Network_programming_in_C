// paral_echo_TCP_client_IPv4_threads.c
#include "unpthread.h"
#include <stdarg.h>
...
static int sockfd;		/* global for both threads to access */         /* глобпльная переменная, доступная обоим потокам */
static FILE *fp;
...
void *copyto(void *arg)
{
	...
	shutdown(sockfd, SHUT_WR);	/* EOF on stdin, send FIN */          /* признак конца файла в стандартном потоке ввода, отправка сегмента FIN */

	return(NULL);
		/* 4return (i.e., thread terminates) when EOF on stdin */    /* завершение потока происходит, когда в стандартном потоке ввода встречается признак конца файла */
}
...
void str_cli(FILE *fp_arg, int sockfd_arg)
{
	...
	sockfd = sockfd_arg;	/* copy arguments to externals */   /* копирование аргументов во внешние переменные */
	fp = fp_arg;
...
int main(int argc, char **argv)
{
   ...
   if(argc != 3)
       err_quit("usage: paral_echo_TCP_client_IPv4_threads <IPaddress> and number port");
