...
int main(int argc, char **argv)
{
   ...
   for(;;){
      ...
      len = sizeof(cliaddr); 
      connfd = Accept(listenfd, (SA *) &cliaddr, &len);                // Прием клиентского соединения. Функция accept() использует системный вызов accept, который имеет номер
