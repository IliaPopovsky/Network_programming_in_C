...
int main(int argc, char **argv)
{
   ...
   for(i = 0; i < 5; i++){
      if((sockfd[i] = socket(AF_INET, SOCK_STREAM, 0)) < 0)            // Создание сокета, функция socket() создает объект сокета в ядре ОС с помощью системного вызова socket, который имеет номер
      ...
      len = sizeof(cliaddr);  // = MAXSOCKADDR;                                                                   
      if(getsockname(sockfd[i], (SA *)&cliaddr, &len) < 0)
